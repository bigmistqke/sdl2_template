...
SDLConsole::SDLConsole()
{
  ...
  // Define the relative path to your TTF font file from the base path
  const char* fontRelativePath = "assets/FreeMono.ttf"; // Adjust this path as needed

  // Construct the full path to the TTF font file
  std::string fontFullPath = std::string(basePath) + fontRelativePath;

	m_font = TTF_OpenFont(fontFullPath.c_str(), 16);
	if (m_font == nullptr)
	{
		std::cerr << "TTF_OpenFont failed." + fontFullPath << std::endl;
		SDL_Quit();
		exit(1);
	}
